    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="mt-8 space-y-5">
          ...
          <label className="block">
            <div className="text-sm text-slate-300 mb-1.5">행사일</div>
          ...
          <div className="text-sm text-slate-400 bg-slate-900/40 border border-slate-800 rounded-lg p-3">
            <span className="text-slate-200 font-medium">기본 X-배너 1장</span>이 자동으로 추가됩니다. 필요한 만큼 에디터에서 추가하세요.
          </div>

          <button onClick={handleCreate} disabled={!eventName.trim() || loading}
            className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-slate-700 hover:bg-slate-600 px-4 py-3 font-medium text-white disabled:opacity-50">
...
const inputCls = 'w-full bg-slate-900 border border-slate-700 rounded-lg px-3.5 py-2.5 text-slate-100 placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-slate-500'
