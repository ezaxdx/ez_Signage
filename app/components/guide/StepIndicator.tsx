...
export function StepIndicator({ steps = DEFAULT_STEPS, current, className = '' }: Props) {
  return (
    <div className={`sticky top-0 z-30 bg-slate-950/85 backdrop-blur-md border-b border-slate-200 ${className}`}>
            ...
            <div key={step.id} className="flex items-center gap-2 flex-shrink-0">
              <div className={`flex items-center gap-2 ${active ? 'text-indigo-300' : done ? 'text-emerald-400' : 'text-slate-500'}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${
                  active ? 'bg-indigo-600 text-white' : done ? 'bg-emerald-600 text-white' : 'bg-slate-50 text-slate-400'
                }`}>
                  {done ? <Check className="w-3.5 h-3.5" /> : idx + 1}
                </span>
                <span className="text-sm whitespace-nowrap">{step.label}</span>
              </div>
              {idx < steps.length - 1 && <div className={`h-px w-8 ${done ? 'bg-emerald-600/50' : 'bg-slate-50'}`} />}
            </div>
          )
        })}
      </div>
