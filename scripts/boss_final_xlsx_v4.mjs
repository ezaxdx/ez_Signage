#!/usr/bin/env node
// 상사 요청 최종본 v4 (2026-05-19)
// 변경 (v3 → v4):
//   1. "학습 크기" → "학습 내용" 변경
//   2. 학습 내용 = 크기 + 영역(외벽·기둥·천장·로비 등) + 파일 형식(CAD·시안·발주 엑셀·PDF)
//   3. 시트 1·2 동일 컬럼 정합
...
const OUT_PATH = 'C:/Users/EZPMP/Desktop/클로드 코드 활동용/★확인/학습데이터_상사요청_최종본_v4_260519.xlsx'
...
// 학습 내용 = 크기 + 영역 + 파일 형식
function buildLearningContent(name) {
  const parts = []

  // 1. 크기 (mm × mm)
  const sizeM = name.match(/(\d{3,5})\s*[x×*]\s*(\d{3,5})/)
  if (sizeM) parts.push(`${sizeM[1]}×${sizeM[2]}mm`)

  // 2. 영역 (외벽·기둥·천장·로비·게이트 등)
  const areaPatterns = [
    { pat: /외벽|outer|입구상단|입구 상단/, label: '외벽·입구' },
    { pat: /원형기둥|사각기둥|기둥배너|로비기둥/, label: '기둥' },
    { pat: /천장|천정|행잉|상단/, label: '천장' },
    { pat: /로비/, label: '로비' },
    { pat: /게이트|gate/i, label: '게이트' },
    { pat: /동선|유도/, label: '동선' },
    { pat: /에스컬레이터|계단|엘리베이터|유리벽/, label: '계단·에스컬레이터' },
    { pat: /무대|단상|개막식/, label: '무대·단상' },
    { pat: /포토월|기념촬영|포토존/, label: '포토존' },
    { pat: /난간/, label: '난간' },
    { pat: /등록|데스크|환영/, label: '등록·환영' },
    { pat: /테이블|좌석|배치도/, label: '좌석 배치' },
    { pat: /시상|어워드/, label: '시상' },
    { pat: /부스|booth/i, label: '부스' },
  ...
  // 3. 파일 형식
  if (/\.(dwg|dxf)$/i.test(name)) parts.push('CAD')
  else if (/\.(ai|psd)$/i.test(name)) parts.push('시안')
  else if (/\.(xlsx?|csv)$/i.test(name)) parts.push('발주 엑셀')
  else if (/\.pdf$/i.test(name)) parts.push('PDF')
  else if (/\.(jpg|jpeg|png|webp)$/i.test(name)) parts.push('실사 이미지')
  else if (/\.(doc|docx|hwp)$/i.test(name)) parts.push('문서')
...
for (const venueName of venueDirs) {
  ...
  function pushFile(file, source, l2Hall, eventCode, eventName, area) {
    allFiles.push({
      venue: venueName,
      source, l2_hall: l2Hall || '', event_code: eventCode || '', event_name: eventName || '', area: area || '',
      file_name: file.name,
      learn_category: classifyByFileName(file.name),
      learn_content: buildLearningContent(file.name),
      rel_path: file.rel,
    })
  }
  ...
  function walk(dir) {
    for (const e of safeReadDir(dir)) {
      ...
      else if (cls === 'application') walkAll(fp, '신청·제출 서류', '', '', '', '')
...
wsM.columns = [{ header: '항목', key: 'a', width: 30 }, { header: '내용', key: 'b', width: 110 }]
const metaData = [
  ['파일명', '학습데이터_상사요청_최종본_v4_260519.xlsx'],
  ['조사 일자', '2026-05-19'],
  ['소스 (SOT)', 'G:/내 드라이브/2026년_AXDX팀/01. AI 업무 파트너/05. 제작물 리스트 가이드/AI 학습자료/L1_행사장'],
  ['', ''],
  ['시트 구성', ''],
  ['  시트 1', '행사장 요약 — 행사장 1행 = 카운트·도면 파일명·행사명·학습 카테고리·학습 내용 분포'],
  ['  시트 2', '학습 파일 상세 (long format) — 파일 1건 = 1행. 행사장·분류·카테고리·내용별 자동 필터'],
  ['', ''],
  ['v3 → v4 변경', ''],
  ['  ① 컬럼 명', '"학습 크기" → "학습 내용" (사용자 명시)'],
  ['  ② 학습 내용 구성', '크기(mm × mm) + 영역(외벽·기둥·천장·로비 등 19 패턴) + 파일 형식(CAD·시안·발주 엑셀·PDF·실사)'],
  ['', ''],
  ['상사 요청 매핑', ''],
  ['  ① 행사장', '시트 1·2 B열'],
  ['  ② 기본 도면', '시트 2 분류 = "기본 도면" 필터'],
  ['  ③ 진행 행사', '시트 2 분류 = "진행 행사 학습" 필터 + 행사명·L2 컬럼'],
  ['  ④ 어떤 정보 학습됐는지', '시트 2 학습 카테고리·학습 내용 컬럼 자동 추출'],
  ['  ⑤ 어디서 가져왔는지', '시트 2 파일 경로 (L1_행사장 이후)'],
  ['', ''],
  ['자동 카테고리', '22 패턴 — 통천·가로 현수막·세로 현수막(난간·기둥·바톤)·X배너·I배너·가로등 배너·포디움·동선 배너·A3·A4·CAD 도면·시설 가이드'],
  ['자동 영역 추출', '19 패턴 — 외벽·기둥·천장·로비·게이트·동선·계단·무대·포토존·난간·등록·좌석·시상·부스·광장·연회·공항·웰컴·개회식'],
...
wsSum.columns = [
  ...
  { header: '학습 내용 분포 (영역·크기)', key: 'content_dist', width: 60 },
]
...
for (const [venue, files] of Object.entries(venueGroups).sort((a, b) => a[0].localeCompare(b[0]))) {
  ...
  files.forEach(f => {
    if (f.learn_content === '—') return
    f.learn_content.split(' · ').forEach(t => contentTokens.set(t, (contentTokens.get(t) ?? 0) + 1))
  })
  const contentDist = [...contentTokens.entries()].sort((a, b) => b[1] - a[1])
    .map(([t, n]) => `${t} ${n}`).join(' · ') || '—'
  ...
  const row = wsSum.addRow({
    ...
    cat_dist: catDist, content_dist: contentDist,
  })
  row.alignment = { vertical: 'top', wrapText: true }
  ...
  const lines = Math.max(dFilesList.split('\n').length, eNamesList.split('\n').length,
    catDist.split(' · ').length, contentDist.split(' · ').length, 1)
...
wsDet.columns = [
  ...
  { header: '학습 내용', key: 'content', width: 40 },
...
console.log('✓ v4 상사 요청 최종본 작성 완료 ("학습 크기" → "학습 내용")')
