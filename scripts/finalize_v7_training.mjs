// v7 = v6 + Vision summary 활용 + 이미지 경로 매칭 + VENUE_LIST 위치 메타 보강
...
// Vision/OCR summary 영역 활용 (items 빈 응답·summary는 있음)
const visionMap = new Map()
if (existsSync(VISION_PATH)) {
  try {
    for (const r of JSON.parse(readFileSync(VISION_PATH, 'utf-8'))) {
      // path = Windows backslash → forward slash 통일 후 rel 추출
...
console.log('v7 = 면적 메타·이미지 경로·Vision summary 보강\n')

const files = walk(DEST)
const filteredFiles = files.filter(f => f.path.replace(/\\/g, '/').replace(DEST + '/', '').includes('/'))
console.log(`잔존 = ${files.length}건·행사장 폴더 안 = ${filteredFiles.length}건`)

// 행사 코드별 이미지/시안 파일 인덱싱
const imagesByEventCode = new Map() // code → [imagePath]
for (const f of filteredFiles) {
  const rel = f.path.replace(/\\/g, '/').replace(DEST + '/', '')
  if (!['jpg', 'jpeg', 'png', 'gif', 'pdf'].includes(f.ext)) continue
  const parts = rel.split('/')
  const eventMatch = parts.find(p => /\d{6}/.test(p)) ?? ''
  const codeMatch = eventMatch.match(/\d{6}(-\d)?/)?.[0]
  if (codeMatch) {
    if (!imagesByEventCode.has(codeMatch)) imagesByEventCode.set(codeMatch, [])
    imagesByEventCode.get(codeMatch).push(rel)
  }
}
console.log(`이미지/시안 인덱스 = ${imagesByEventCode.size} 행사 코드·총 ${[...imagesByEventCode.values()].flat().length} 파일`)
...
for (const f of filteredFiles) {
  ...
  for (const hall of halls) {
    fileRows.push({ event_code: eventCode, event_name: eventName, l1, hall: hall || '미상', year, month: '미상', date: '미상', file: f.name, path: rel, note })
  }

  if (f.ext === 'xlsx' && /제작물|환경|발주|리스트/.test(f.name)) {
    const items = parseOrderXlsx(f.path)
    for (const it of items) {
      // 사용자 명시 (5/21) = "전부 적도록" = 필터 X
      for (const hall of halls) {
        // 이미지 경로 매칭 = 같은 행사 코드 안 jpg/png/pdf 중 카테고리 매칭
        let imgPath = '미상'
        if (codeMatch && imagesByEventCode.has(codeMatch)) {
          const candidates = imagesByEventCode.get(codeMatch)
          const catKey = it.category_12
          // 파일명에 카테고리 키워드 포함된 영역 우선
...
console.log(`시트 2 행 = ${fileRows.length}·시트 3 발주 = ${orderRows.length}`)
const imgMatched = orderRows.filter(r => r.image_path !== '미상').length
const visionMatched = orderRows.filter(r => r.vision_raw !== '미상').length
console.log(`이미지 경로 매칭 = ${imgMatched}/${orderRows.length}·Vision summary 매칭 = ${visionMatched}`)

const wb = xlsx.utils.book_new()

// 시트 1 = 행사장_요약 + VENUE_META 보강
const s1 = [['행사장명', '위치', '주요 홀', '행사 건수', '환경장식물 행 수', '카테고리 분포', '평균 면적', '최초 행사 연도', '최근 행사 연도', '비고']]
VENUE_REGISTRY.forEach(v => {
  const s = venueStats[v.std] || { halls: new Set(), events: new Set(), signageRows: 0, signageDist: {}, minYear: null, maxYear: null }
...
s4.push(['처리 — 발주 행 (전부 진입)', orderRows.length + '건', '', '', ''])
s4.push(['처리 — 총 환경장식물 수량', totalSig + '개', '', '', ''])
s4.push(['처리 — 이미지 경로 매칭', `${imgMatched}/${orderRows.length}건`, '', '', ''])
s4.push(['처리 — Vision summary 매칭', `${visionMatched}건`, '', '', ''])
s4.push(['처리 — L1 행사장 (PR#3 + 롯데호텔 제주)', VENUE_REGISTRY.length + '개', '', '', ''])
s4.push(['v7 보강 영역', '위치 메타 33건·이미지 경로 매칭·Vision summary', '', '', ''])
xlsx.utils.book_append_sheet(wb, xlsx.utils.aoa_to_sheet(s4), '삭제_리스트')

xlsx.writeFile(wb, OUT)
console.log(`\n완료: ${OUT}`)
console.log(`  · 시트 1: ${VENUE_REGISTRY.length} L1 (위치 메타 33건)`)
console.log(`  · 시트 2: ${fileRows.length}건`)
console.log(`  · 시트 3: ${orderRows.length}건·총 수량 ${totalSig}개·이미지 매칭 ${imgMatched}건`)
