...
**선택**:
  1. **B3 = RightPanel 신규 컴포넌트** = EditorLayout 우 2 영역 CanvasBoard 교체. 예시 이미지(sample_image_url 옵셔널·향후 데이터허브 연동) + 사이즈 비율 시각화 + 위반 사항(facilityIssueMap[selectedItemId] 자동 분기).
  2. **CanvasBoard orphan 보존** = decisions.md 2026-05-07 orphan 정책 정합. 시안 캔버스 부활 시 재사용.
  3. **B6 = VENUE_HALLS 정적 시드 1차** = lib/venueIntel.ts에 노션 §9 시드 25건 추가. 관리자 UI = read-only VenueHierarchyTree (L1 펼침·접힘).
  4. **추가/수정/삭제 = Supabase venue_halls 마이그레이션 후 활성** = v6에 이미 venue_halls 테이블 존재. 1차는 정적 시드만 시각 노출·DB INSERT는 컴펌 후.

**대안**:
  - CanvasBoard 완전 삭제 (v5 부활 가능성 차단·orphan 정책 위반)
  - VENUE_HALLS DB INSERT 자동 (사용자 컴펌 X 상태 라이브 영향 발생 위험)
  - 예시 이미지 업로드 UI 동시 작업 (관리자 페이지 "환경 장식물 종류" 메뉴 큰 변경·사이클 부담)

**이유**:
  - 사용자 "전부 진행" 명시 = 박제 자제 룰 자동 무력화 (CLAUDE.md §7 #18·feedback-사용자명시-우선원칙-260519)
  - orphan 정책 = 회전 가능성·코드 보존 비용 < 삭제 후 복구 비용 (decisions.md 2026-05-07)
  - 정적 시드 1차 = 라이브 영향 0건·사용자 시각 확인 가능·D-day 안전 (5/22 D-1 영역 운영 안정)
  - 노션 SOT 직접 반영 = §3·§9 그대로·해석 X (외부 자료 = 입력 자료 영역만 사용 룰)

**되돌릴 수 있는가**: 매우 쉬움 — RightPanel·VenueHierarchyTree 두 컴포넌트 제거 + EditorLayout 1줄 환원 + VENUE_HALLS 시드 삭제. 코드 영향 4 파일.

**관련**: 노션 페이지 36148589-8ea1-81a3-b3e8-dd4a833c914c §3·§9·docs/diff_260520.md B3·B6·decisions.md 2026-05-07 orphan 정책

---

## 2026-05-19 — 상사 보고 자료 분류 룰 5중 박제 + "영역" 과사용 회피 + check_v3 SEED_SYNONYMS 점검 (v10.1)

**컨텍스트**: 5/19 PO 명시 2건 = (1) "검토인지 의사결정인지 보고인지 같은 거 알 수 있게·이거 어딜 봐요·뭔 내용·뭘 말함 질문 안 나와야" (2) "영역같은거 더 문제 없게 하고". 5/19 v10.0 환경장식물 정합 후 후속 영역 강화.

**선택**:
  1. **자료 분류 4종 SOT** = `[검토 요청]`·`[의사 결정 요청]`·`[단순 보고]`·`[진척 공유]` 4 분류 첫 줄·제목 명시 의무
  2. **5중 박제** = 메모리 신규 + CLAUDE.md §2·§7 #16·#22 + MEMORY.md 인덱스 + 5 가이드 §0 메타 룰 + learnings.md 5번째
  3. **5 가이드 정합** = 회의록·결재·메일·보고서·IA 가이드 §0 메타 룰 추가 (기획안 templates·사내행정 = 후속)
  4. **"영역" 과사용 회피 룰** = 메모리 `feedback-sot-260518` 영역 영역 추가·5회 이하 자체 카운트·CLAUDE.md §7 #23
  5. **scripts/check_v3 확장** = SEED_SYNONYMS 정합 점검 신규 (노션 §8-1 핵심 6건·대시 표기 잔존) = 19 → 21 점검 / 0 fail

**대안**:
  - 자료 분류 명시 X·BLUF만 (사용자 명시 위반)
  - 1중 박제 (메모리만) = 가이드 SOT 영역 미정합·실 자료 적용 미보장
  - "영역" → 다른 단어 일괄 치환 = 자모 노이즈 재발 가능 (5/18 "영원" 패턴 정합)
  - check_v3 영역 X = 재발 회피 도구화 미진행

**이유**:
  - PO 명시 = 상사 "어딜 봐요·뭔 내용" 질문 0건 영구 회피 = 5중 박제 정합 영역
  - 5/19 학습 #9 영역 = "박제만 X·실 자료 적용" = 5 가이드 §0 추가 의무
  - "영역" = 5/18 "영원" 노이즈 회피 후 새 패턴 = 자체 카운트 룰 영구 = AI 어투 회피
  - check_v3 = Reflexion 영역·자동 검증·다음 사이클 누락 자동 발견

**되돌릴 수 있는가**: 매우 쉬움 — 메모리 1건 삭제·가이드 §0 영역 5건 정정·CLAUDE.md §2·§7 #16·#22·#23 4 줄 제거. 코드 영역 영향 0건.

**관련**: feedback-상사보고-분류명시-의무-260519·feedback-sot-260518·CLAUDE.md §7 22·23 질문·5 가이드 §0 메타 룰
...
**선택**:
  1. **AdminOpsClient.tsx에서 두 섹션 JSX 블록 + 관련 인터페이스 + Calendar 아이콘 import 제거** — `PartBar`·`CalendarItem` 인터페이스 + `partStageBars`·`calendar` props + ′파트별 상태 Stacked Bar′ 섹션 + ′이번 달 일정 캘린더′ 섹션 일괄 제거. 한 의미 단위로 묶어 단일 변경.
  2. **page.tsx에서 partStageBars·calendarItems 데이터 산출 로직 제거** — `partStageMap` 집계 루프(~16줄) + `calStart`/`calEnd` Date 변수 + `calendarItems` filter+map+sort(~22줄) 제거. AdminOpsClient prop 전달부에서도 두 키 제외. v9.47에서 보존했던 `void weekStart`·weekStart 변수도 함께 제거(이전 보존 사유 사라짐).
  3. **섹션 라벨 ′전체 프로젝트 현황′ → ′프로젝트 등록 현황′ 정합** — 사용자 IA SOT 명시 영역 명칭과 일치시켜 시각·코드 두 곳에서 같은 표현 사용.

**대안**:
  - 두 섹션을 `false &&` 가드로 비활성만 (코드 잔존 — 향후 ′응?′ 룰 위반 가능, dead code 누적)
  - 두 섹션을 별도 어드민 라우트(`/admin/insights`)로 분리 (IA에 없는 신규 라우트 신설 — 사용자 명시 ′삭제′ 의도 위반)
  - 캘린더는 유지하고 Stacked Bar만 삭제 (IA SOT는 둘 다 미포함 — 부분 보존은 일관성 깨짐)

**이유**:
  - **IA SOT 단일 진실 소스**: 김연아 대리님 스크린샷이 검토자 명시 결정사항. 코드와 IA 차이가 누적되면 다음 화면 추가 시 기준점 모호 (decisions.md 2026-05-14 v9.47과 같은 원칙).
  - **운영 대시보드 본질 = 1차 진단**: 운영 KPI 4 + 프로젝트 등록 표만으로 "지금 진행 상황이 어떤가" 1차 판단 가능. Stacked Bar·캘린더는 분석 자료(`/data` 13탭) 역할로 충분.
  - **사용자 ′응?′ 룰 정합**: 화면에 의미 불명 영역이 있으면 사용자가 "이건 왜 있나?" 질문 발생. IA에 없는 영역은 즉시 정리하는 게 일관.
  - **삭제 안전**: 두 영역은 partStageBars·calendarItems 단방향 prop만 사용 — 다른 컴포넌트가 의존 0건. 인터페이스 + JSX + 데이터 산출만 깔끔히 제거 가능.
...
**보존**:
  ...
  - DB 스키마 변경 0건 / 의존성 추가 0건
  - v9.48 admin AI 페이지 작업과 파일 겹침 0건 (다른 파일)

**되돌릴 수 있는가**:
  - 매우 쉬움 — git revert 또는 stash@{0} 재적용으로 두 섹션 + 데이터 산출 로직 복원. AdminOpsClient·page.tsx 두 파일만 영향.

**상세**: PROGRESS.md 2026-05-14 (v9.49)
