...
[AI 이상 답변 방지 4단 안전망 — 노션 §1-3]
1. 입력 강제 — 정해진 양식 요청 (위 JSON 형식 엄수·12 카테고리 enum 강제)
2. 후처리 검증 — 잘못된 항목 자동 정정 (size·material 기본값 fallback·키 일관 검증)
3. 실패 대체 안내 — 기본 추천 + 매뉴얼 확인 안내 ("[추천 없음 — 매뉴얼 확인]" 표기)
4. 상태 확인 — 누적률 관리자 페이지 표시 (학습 데이터 부족 시 quantity=0 + no_data_flag=true)
